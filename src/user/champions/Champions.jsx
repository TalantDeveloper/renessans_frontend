...
const ChampionsBoard = () => {
    ...
    const renderTrophyIconAndStars = (description) => {
        if (!description) return {icon: <FaTrophy className={styles.icon}/>, stars: 2}; // Return default if description is undefined

        const lowerCaseDescription = description.toLowerCase();
        if (lowerCaseDescription.includes("oltin")) {
            return {
                icon: <FaMedal className={styles.icon} style={{color: "#FFD700"}}/>, stars: 5,
            };
        } else if (lowerCaseDescription.includes("kumush")) {
            return {
                icon: <FaMedal className={styles.icon} style={{color: "#C0C0C0"}}/>, stars: 4,
            };
        } else if (lowerCaseDescription.includes("bronza")) {
            return {
                icon: <FaMedal className={styles.icon} style={{color: "#CD7F32"}}/>, stars: 3,
            };
        } else {
            return {
                icon: <FaTrophy className={styles.icon}/>, stars: 2,
            };
        }
    };

    // Render stars dynamically
    const renderStars = (count) => Array.from({length: count}).map((_, idx) => (
        <FaStar key={idx} className={styles.star}/>));

    return (<section className={styles.championBoard}>
            <div className={styles.container}>
                <h2 className={styles.title}>{t("championBoardTitle")}</h2>{" "}
                {/* Use translation */}
                <div className={styles.championGrid}>
                    {champions.map((champion) => {
                        const {icon, stars} = renderTrophyIconAndStars(champion[`description_${i18n.language}`] // Dynamically use the selected language description
                        );
                        return (<div key={champion.id} className={styles.championCard}>
                                <div className={styles.cardHeader}>
                                    <img
                                        src={champion.image}
                                        alt={`${champion[`name_${i18n.language}`]} ${champion[`surname_${i18n.language}`]}`} // Dynamically display name and surname
